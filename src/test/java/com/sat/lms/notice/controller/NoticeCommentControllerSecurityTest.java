...
class NoticeCommentControllerSecurityTest {
    ...
    void authenticatedStudentCanCreateComment() throws Exception {
        ...
        NoticeCommentResponse response = response();
        when(noticeCommentService.create(eq(1L), eq(8L), any())).thenReturn(response);
    ...
    @Test
    void authenticatedAdminCanCreateComment() throws Exception {
        authenticate("admin", 9L, "ADMIN");
        NoticeCommentResponse response = response();
        when(noticeCommentService.create(eq(1L), eq(9L), any())).thenReturn(response);
    ...
    void authenticatedStudentCanListComments() throws Exception {
        authenticate("student", 8L, "STUDENT");
        Page<NoticeCommentResponse> page = new PageImpl<>(List.of(response()));
        when(noticeCommentService.getComments(eq(1L), eq(8L), any(Pageable.class))).thenReturn(page);
    ...
    @Test
    void authenticatedAdminCanListComments() throws Exception {
        authenticate("admin", 9L, "ADMIN");
        Page<NoticeCommentResponse> page = new PageImpl<>(List.of(response()));
        when(noticeCommentService.getComments(eq(1L), eq(9L), any(Pageable.class))).thenReturn(page);
    ...
    void authenticatedAuthorCanUpdateComment() throws Exception {
        authenticate("student", 8L, "STUDENT");
        NoticeCommentResponse response = response();
        when(noticeCommentService.update(eq(1L), eq(8L), any())).thenReturn(response);
    ...
    private NoticeCommentResponse response() {
