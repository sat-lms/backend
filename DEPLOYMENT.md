...
회원 자진 탈퇴 또는 관리자 추방 시 해당 회원의 토큰 버전이 증가합니다. 이후 계정 복구 및 관리자 재승인이
완료되어도 탈퇴·추방 전에 발급된 JWT는 다시 유효해지지 않으며, 사용자는 재승인 후 새로 로그인해야 합니다.
토큰 버전 claim이 없는 배포 이전 JWT도 보안을 우선하여 거부되므로 이 변경 배포 후 기존 사용자의 재로그인이
필요합니다. 토큰 버전은 DB에 저장되므로 서버 재시작이나 여러 애플리케이션 인스턴스에서도 동일하게 적용됩니다.
